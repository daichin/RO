...
ESPlorer 是逐行送的，串列埠一忙就可能掉行 —— **而且不會報錯**。曾經因此把 `init.lua` 截斷在第 79 行，開機直接噴 `'then' expected near '<eof>'`。

每次上傳完在序列埠核對一次：

```lua
print(file.list()["init.lua"])
```

數字要跟本機的檔案大小一模一樣。不一樣就重傳。

`init.lua` 特別脆弱，因為它是**唯一必須留在 SPIFFS、不能編譯、也不能放進 LFS** 的檔案（NodeMCU 開機時按檔名找它），每次改動都得重走一次序列埠。所以這個檔刻意保持精簡，設計理由都寫在本文件而不是塞在程式碼註解裡。
